...
impl SExpression {
    ...
    pub fn get_mut(&mut self, path: &str) -> Option<&mut SExpression> {
        let parts = path.split(':');
        self.get_for_parts_mut(parts)
    }
    ...
    fn get_for_parts_mut<'a, I>(&mut self, parts: I) -> Option<&mut SExpression>
    where
        I: IntoIterator<Item = &'a str>,
    {
        let mut iter = parts.into_iter();
        ...
        match iter.next() {
            Some("") => self.get_for_parts_mut(iter),
            Some(name) => match self {
                SExpression::Object(maybe_name, fields)
                    if maybe_name.as_ref().is_some_and(|n| n == name) || maybe_name.is_none() =>
                {
                    ...
                    match name {
                        Some(name) => {
                            for (f_name, f_value) in fields {
                                if f_name == name {
                                    return f_value.get_for_parts_mut(iter);
                                }
                            }
                            None
                        }
